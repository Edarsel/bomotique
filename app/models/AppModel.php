<?php
class AppModel extends connexionBD
{
  ...
  function getLogsConnexion() {
    ...
    function getLogsConnexionParUtilisateur($idUtil) {
      $bdd = $this->getBdd();
      $stmt = $bdd->prepare('SELECT `tbl_log_connexion`.*, `tbl_utilisateur`.`nomUtilisateur`
        FROM `tbl_log_connexion`
        RIGHT JOIN `tbl_utilisateur` ON `tbl_log_connexion`.`num_tbl_utilisateur` = `tbl_utilisateur`.`numero`
        WHERE `tbl_utilisateur`.`numero` = :numUtil ORDER BY `tbl_log_connexion`.`numero` DESC');
        $stmt->bindParam(':numUtil', $idUtil, PDO::PARAM_INT);
        $stmt->execute();
        $listeLogs = $stmt->fetchAll(PDO::FETCH_OBJ);
        return $listeLogs;
      }
